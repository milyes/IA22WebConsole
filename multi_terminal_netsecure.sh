...
while true; do
    ...
    echo "9. Quitter"
echo "20. Lancer le mini terminal IA avancé (Python)"

echo "18. Détecter les exécutables dans $HOME et ~/downloads"
...
echo "15. Lancer FastAPI Emprunts IA (port 8000)"
    ...
    15)
        ...
        if [ -d "$HOME/fastapi_emprunts_netsecure/app" ]; then
            cd "$HOME/fastapi_emprunts_netsecure/app"
        elif [ -d "$HOME/downloads/fastapi_emprunts_netsecure/app" ]; then
            cd "$HOME/downloads/fastapi_emprunts_netsecure/app"
        else
            echo "❌ Projet non trouvé ni dans $HOME ni dans ~/downloads"
            break
        fi
    ...
    17)
        if [ -f "$HOME/fastapi_emprunts_netsecure/app/emprunts.db" ]; then
            DB_PATH="$HOME/fastapi_emprunts_netsecure/app/emprunts.db"
        elif [ -f "$HOME/downloads/fastapi_emprunts_netsecure/app/emprunts.db" ]; then
            DB_PATH="$HOME/downloads/fastapi_emprunts_netsecure/app/emprunts.db"
        else
            echo "❌ Base de données introuvable."
            break
        fi
        echo "📂 Contenu de la base de données SQLite :"
    ...
    18)
        echo "🔎 Lancement de la détection des exécutables dans $HOME et ~/downloads..."
        bash ~/detect_exec.sh
        ;;
        
    20)
        echo "🧠 Lancement du mini terminal IA en Python..."
        if [ -f "$HOME/mini_terminal_avance.py" ]; then
            python3 "$HOME/mini_terminal_avance.py"
        elif [ -f "$HOME/downloads/mini_terminal_avance.py" ]; then
            python3 "$HOME/downloads/mini_terminal_avance.py"
        else
            echo "❌ Fichier mini_terminal_avance.py introuvable."
        fi
        ;;
        *) echo "❌ Choix invalide." ;;
    esac
